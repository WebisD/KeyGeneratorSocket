...
class Statistics:
    def __init__(self, file_path=None, std=False) -> None:
        if file_path != None:
            ...
            if not std:
                ...
                for _ in range(0,10):
                    temp = []
                    for i in range(0,len(times_str),10):
    ...
    def return_xy(self):
        ...
        if not self.std:
            for i in range(len(self.times)):
                x_time.append(self.times[i] if i == 0 else x_time[i-1] + self.times[i])
                y_count.append(i+1)
    ...
    def plot_LUT_graph(self, title, file_name):
        ...
        plt.annotate(f"({round(time,6)}, {index+1})", (time, index+1), xytext=(time-1.5, index+10000))
    ...
    def plot_std_graph(self, title, file_name):
        ...
        #plt.plot(x_time, y_count)
        plt.errorbar(x_time, y_count, error, fmt='-')
